...
float2 ConvertOriginalRawUVToInteriorUV(const float2 originalRawUV, const float3 viewDirTangentSpace, const float roomMaxDepth01Define)
{			
	//remap [0,1] to [+inf,0]
	...
	const float depthScale = rcp(roomMaxDepth01Define) - 1.0;
	...
	const float3 viewRayStartPosBoxSpace = float3(originalRawUV * 2 - 1, -1); //normalized box space's ray start pos is on trinagle surface, where z = -1
	const float3 viewRayDirBoxSpace = viewDirTangentSpace * float3(1, 1, -depthScale);//transform input ray dir from tangent space to normalized box space
	...
	//============================================================================
	const float3 viewRayDirBoxSpaceRcp = rcp(viewRayDirBoxSpace);
	...
	const float shortestHitRayLength = min(min(hitRayLengthForSeperatedAxis.x, hitRayLengthForSeperatedAxis.y), hitRayLengthForSeperatedAxis.z);
